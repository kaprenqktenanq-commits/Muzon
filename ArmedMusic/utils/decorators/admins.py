...
from ArmedMusic .utils .database import get_authuser_names ,get_cmode ,get_lang ,get_upvote_count ,is_active_chat ,is_maintenance ,is_nonadmin_chat ,is_skipmode
            ...
            if message .from_user .id not in SUDOERS :
                ...
                if not admins :
                    ...
                    if await is_skipmode (message .chat .id ):
                        ...
                        MODE =command .title ()
                        upl =InlineKeyboardMarkup ([[InlineKeyboardButton (text ='ᴠᴏᴛᴇ',callback_data =f'ADMIN  UpVote|{chat_id }_{MODE }')]])
    ...
    async def wrapper (client ,CallbackQuery ):
        if await is_maintenance ()is False :
            if CallbackQuery .from_user .id not in SUDOERS :
                return await CallbackQuery .answer (f'{app .mention } ɪs ᴜɴᴅᴇʀ ᴍᴀɪɴᴛᴇɴᴀɴᴄᴇ, ᴠɪsɪᴛ sᴜᴘᴘᴏʀᴛ ᴄʜᴀᴛ ғᴏʀ ᴋɴᴏᴡɪɴɢ ᴛʜᴇ ʀᴇᴀsᴏɴ.',show_alert =True )
        try :
