...
from ArmedMusic.utils.database import get_active_chats, get_authuser_names, get_client, get_served_chats, get_served_users
from ArmedMusic.utils.decorators.language import language
from ArmedMusic.utils.formatters import alpha_to_int
...
async def braodcast_message(client, message: Message, _):
    ...
    if '-assistant' in message.text:
        ...
        text = _['broad_6']
        from ArmedMusic.core.userbot import assistants
...
async def auto_clean_cache():
    ...
    while not await asyncio.sleep(CACHE_SLEEP):
        try:
            ...
            expired_files = [file_path for file_path, last_access in file_cache.items() if current_time - last_access > CACHE_DURATION and file_path not in autoclean]
