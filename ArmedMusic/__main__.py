...
from ArmedMusic.utils.error_handler import ErrorHandler, handle_unknown_constructor
...
async def init():
    ...
    # Start main bot with retry logic
    try:
        await app.start()
    ...
    # Start userbot with retry logic
    try:
        await userbot.start()
        ...
        # Don't exit, continue with main bot
...
def _handle_loop_exception(loop, context):
    ...
    try:
        ...
        if error:
            # Handle unknown constructor errors
            error_str = str(error)
            if "unknown constructor" in error_str.lower() or "0x" in error_str:
                ...
                # These are usually transient - bot will recover
                return
...
if __name__ == '__main__':
    ...
    # Run init inside a safe wrapper with reconnection logic
    async def _safe_init():
        ...
        while True:
            try:
                ...
                # Check if it's a TL schema error (transient)
                ...
                wait_time = min(5 * (2 ** RECONNECT_ATTEMPTS), 300)  # Cap at 5 minutes
