...
def remove_emojis(text):
    """Remove emojis from text to prevent rendering issues with limited fonts"""
    if not text:
        return text
    # Remove emojis and other non-ASCII characters that fonts might not support
    cleaned = ''
    for char in text:
        try:
            # Check if character is in common ranges
            codepoint = ord(char)
            # Skip emoji ranges (0x1F000 and up) and other symbol ranges
            if codepoint >= 0x1F000:  # Emoticon and symbol ranges
                continue
            if 0x2600 <= codepoint <= 0x27BF:  # Miscellaneous symbols
                continue
            if 0x1F300 <= codepoint <= 0x1F9FF:  # Emoji ranges
                continue
            # Check if it's a valid unicode category
            category = unicodedata.category(char)
            if category.startswith('C'):  # Control, Format, Surrogate, Private Use, Not Assigned
                continue
            cleaned += char
        except:
            continue
    return cleaned.strip()
ASSETS_DIR = Path('ArmedMusic/fonts')
...
async def get_thumb(videoid: str, queue_pos: int=1, title_style: str='bold'):
    ...
    try:
        ...
        max_w = W - info_x - 70
        # Remove emojis from title to prevent rendering issues
