...
from ArmedMusic.utils.database import add_nonadmin_chat, get_authuser, get_authuser_names, get_playmode, get_playtype, get_upvote_count, is_nonadmin_chat, is_skipmode, remove_nonadmin_chat, set_playmode, set_playtype, set_upvotes, skip_off, skip_on
...
from ArmedMusic.utils.inline.settings import auth_users_markup, playmode_users_markup, setting_markup, vote_mode_markup
...
async def settings_mar(client, message: Message):
    from ArmedMusic.utils.database import get_lang
    lang = await get_lang(message.chat.id)
    from strings import get_string
    _ = get_string(lang)
    buttons = setting_markup(_)
    await message.reply_text(_['setting_1'].format(app.mention, message.chat.id, message.chat.title), reply_markup=InlineKeyboardMarkup(buttons))
...
async def settings_cb(client, CallbackQuery):
    ...
    try:
        ...
        pass
    buttons = setting_markup(_)
    return await CallbackQuery.edit_message_text(_['setting_1'].format(app.mention, CallbackQuery.message.chat.id, CallbackQuery.message.chat.title), reply_markup=InlineKeyboardMarkup(buttons))
...
async def settings_back_markup(client, CallbackQuery: CallbackQuery):
    ...
    if CallbackQuery.message.chat.type == ChatType.PRIVATE:
        ...
        return await CallbackQuery.edit_message_text(_['start_2'].format(CallbackQuery.from_user.mention, app.mention), reply_markup=InlineKeyboardMarkup(buttons))
    else:
        buttons = setting_markup(_)
        return await CallbackQuery.edit_message_reply_markup(reply_markup=InlineKeyboardMarkup(buttons))

@app.on_callback_query(filters.regex(pattern='^(SEARCHANSWER|PLAYMODEANSWER|PLAYTYPEANSWER|AUTHANSWER|ANSWERVOMODE|VOTEANSWER|PM|AU|VM)$') & ~BANNED_USERS)
...
@app.on_callback_query(filters.regex(pattern='^(MODECHANGE|CHANNELMODECHANGE|PLAYTYPECHANGE)$') & ~BANNED_USERS)
@ActualAdminCB
...
@app.on_callback_query(filters.regex(pattern='^(AUTH|AUTHLIST)$') & ~BANNED_USERS)
@ActualAdminCB
