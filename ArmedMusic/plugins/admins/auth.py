...
from ArmedMusic .utils import extract_user ,int_to_alpha
from ArmedMusic .utils .database import delete_authuser ,get_authuser ,get_authuser_names ,save_authuser
from ArmedMusic .utils .decorators import AdminActual ,language
from ArmedMusic .utils .inline import close_markup
...
async def auth (client ,message :Message ,_ ):
    ...
    if token not in _check :
        assis ={'auth_user_id':user .id ,'auth_name':user .first_name ,'admin_id':message .from_user .id ,'admin_name':message .from_user .first_name }
