...
from ArmedMusic .misc import SUDOERS ,db
from ArmedMusic .utils import AdminRightsCheck
from ArmedMusic .utils .database import is_active_chat ,is_nonadmin_chat
from ArmedMusic .utils .decorators .language import languageCB
from ArmedMusic .utils .inline import close_markup ,speed_markup
...
async def playback (cli ,message :Message ,_ ,chat_id ):
    ...
    return await message .reply_text (text =_ ['admin_28'].format (app .mention ),reply_markup =upl )
...
async def del_back_playlist (client ,CallbackQuery ,_ ):
    ...
    try :
        ...
        pass
    mystic =await CallbackQuery .edit_message_text (text =_ ['admin_32'].format (CallbackQuery .from_user .mention ))
    try :
    ...
    if chat_id in checker :
        checker .remove (chat_id )
    await mystic .edit_text (text =_ ['admin_34'].format (speed ,CallbackQuery .from_user .mention ),reply_markup =close_markup (_ ))