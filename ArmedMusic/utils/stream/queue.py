...
async def put_queue_index (chat_id ,original_chat_id ,file ,title ,duration ,user ,vidid ,stream ,forceplay :Union [bool ,str ]=None ):
    if '20.212.146.162'in vidid :
        ...
        dur =0
    put ={'title':title ,'dur':duration ,'streamtype':stream ,'by':user ,'chat_id':original_chat_id ,'file':file ,'vidid':vidid ,'seconds':dur ,'played':0 }
    if forceplay :
        check =db .get (chat_id )
        if check :
            check .insert (0 ,put )
